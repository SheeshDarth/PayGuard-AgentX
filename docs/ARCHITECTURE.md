...
Week 1 (foundation) is implemented and tested offline: Ollama-first LLM layer, extended schemas, SQLite/Kùzu/Chroma stores with fallbacks, and audit coverage of the new artifact types. Weeks 2–4 (supervisor, ReAct+tools, critics, negotiation, MCP server, dashboard) are planned per `docs/ROADMAP.md`. Nothing here is production-audited — the compliance framing is a design target, kept deliberately honest.
