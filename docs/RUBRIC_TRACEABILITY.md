...
|---|---|---|---|---|
| 1 | **User–agent interaction** | Zero-dependency web dashboard: pick a demo scenario, read the agent execution timeline, expand *why* each item was flagged, work the HITL approval queue, one-click HMAC verify with a tamper demo | [`web/server.py`](../web/server.py), [`web/static/`](../web/static/) | Runs on laptop (`python -m web.server`) |
...
`pytest` — **79 passing** across `test_dq_engine`, `test_pipeline` (incl. the end-to-end
...
fallbacks the 79-test suite exercises — the suite runs with no GPU, no network and no keys.
